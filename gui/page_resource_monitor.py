...
class ResourceMonitorPage(tk.Frame):
    ...
    def _build_top_processes_card(self, parent):
        ...
        self._proc_tree = ttk.Treeview(tree_frame, columns=("usage",), height=8)
        self._proc_tree.column("#0", width=250)
        self._proc_tree.column("usage", width=80)
    ...
    def _update_ui(self, metrics, processes, alerts):
        ...
        for proc in processes:
            self._proc_tree.insert("", "end", text=proc["name"],
                                  values=(f"{proc['usage']:.1f}%",))
