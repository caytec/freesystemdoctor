...
def get_drive_type(drive_letter: str) -> str:
    """Returns 'SSD', 'HDD', or 'Unknown' for the given drive letter (e.g. 'C')."""
    if not drive_letter:
        return "Unknown"
    letter = drive_letter[0].upper()

    # Map drive letter → partition disk number → physical disk MediaType
    script = (
    ...
    t = (r.stdout or "").strip().upper()
    if "SSD" in t or "SOLID" in t:
        return "SSD"
    if "HDD" in t or "HARD" in t or "ROTATION" in t:
        return "HDD"
    ...
    script2 = (
        ...
        f"if ($p) {{ "
        f"  $d = Get-PhysicalDisk | Where-Object {{$_.DeviceId -eq $p.DiskNumber}}; "
        f"  if ($d) {{ $d.SpindleSpeed }} "
        f"}}"
    )
    r2 = _run(["powershell", "-NoProfile", "-Command", script2])
    spindle = (r2.stdout or "").strip()
    if spindle == "0":
        return "SSD"
    if spindle.isdigit() and int(spindle) > 0:
        return "HDD"
    return "Unknown"
