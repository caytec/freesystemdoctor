...
def _cpu_worker(duration_sec: float, result_holder: list) -> None:
    """
    Perform floating-point math operations for *duration_sec* seconds.
    Stores (ops_count, elapsed) in result_holder[0].
    """
    ops = 0
    deadline = time.perf_counter() + duration_sec
    x = 1.0
    while time.perf_counter() < deadline:
        # Mix of sqrt, sin, cos — stresses FPU
...
def cpu_benchmark(
    ...
    try:
        ...
        threads = [
            threading.Thread(target=_cpu_worker, args=(duration_sec, holders[i]), daemon=True)
        ...
        total_ops = sum(h[0] for h in holders if h)
