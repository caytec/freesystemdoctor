...
def get_health_scores() -> dict:
    ...
    Returns dict with scores, overall, and top issues populated.
    """
    scores = {
    ...
    scores["top_issues"]    = get_top_issues(5)
    # Aliases used by the GUI page (which reads scores by short keys)
    scores["privacy"]  = scores["privacy_score"]
    scores["space"]    = scores["space_score"]
    scores["speed"]    = scores["speed_score"]
    scores["security"] = scores["security_score"]
    return scores
