...
def get_service_detail(name: str) -> str:
    ...
    r2 = subprocess.run(["sc", "qc", name], capture_output=True, text=True)
    return (r.stdout + "\n" + r2.stdout).strip()


def start_service(name: str) -> bool:
    r = subprocess.run(["net", "start", name], capture_output=True)
    return r.returncode == 0


def stop_service(name: str) -> bool:
    r = subprocess.run(["net", "stop", name], capture_output=True)
    return r.returncode == 0


def set_service_startup(name: str, mode: str) -> bool:
    """mode: auto | manual | disabled"""
    r = subprocess.run(["sc", "config", name, f"start={mode}"], capture_output=True)
    return r.returncode == 0
