        ...
        for idx, prog in enumerate(installed):
            try:
                ...
                if wg:
                    ...
                    # Use KNOWN_APPS only for download URL & winget_id metadata
                    # — never for version comparison (the hardcoded versions
                    # go stale within weeks of release).
                    known = match_known(prog["name"])
                    if known:
                        entry.winget_id = known.get("winget_id", "")
                        entry.download_url = known.get("url", "")
                    # Without a winget upgrade row we cannot reliably detect
                    # outdated state — mark up_to_date if we have any version,
                    # otherwise unknown.
                    entry.status = "up_to_date" if prog["version"] else "unknown"
