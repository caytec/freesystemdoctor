...
def run_scheduled_task(task: ScheduledTask) -> bool:
    ...
    try:
        if task.action == "disk_clean":
            subprocess.run(["python", "-m", "engine.disk_cleaner"], timeout=3600, capture_output=True)
        elif task.action == "registry_clean":
            subprocess.run(["python", "-m", "engine.registry_cleaner"], timeout=600, capture_output=True)
        elif task.action == "defrag":
            subprocess.run(["python", "-m", "engine.smart_defrag"], timeout=7200, capture_output=True)
        elif task.action == "update_check":
            subprocess.run(["python", "-m", "engine.software_updater"], timeout=300, capture_output=True)
        else:
            return False
